...
export default class DemoDriver {
  ...
  seedFeed() {
    ...
    const seed = [
      ...
      { killer: 'V-07 WRAITHLINE', victim: 'ZEN-7', weapon: 'ar', self: true, age: 2.1 },
      { killer: 'V-07 WRAITHLINE', victim: 'ROOKWOOD', weapon: 'ar', headshot: true, self: true, age: 0.6 },
    ];
  ...
  update(dt) {
    ...
    if (this.burst > 0) {
      ...
      if (this.tShot <= 0) {
        ...
        if (w.mag > 0 && !w.reloading) {
          bus.emit(EV.SHOT_FIRED, { weaponId: 'cr56_carbine', spread: 0.2 });
    ...
    if (this.tKill <= 0) {
      ...
      bus.emit(EV.ENTITY_KILLED, {
        id: foe, name: foe, by: 'player', byName: 'V-07 WRAITHLINE',
